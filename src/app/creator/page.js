    ...
    <>
      ...
      <div className="p-4 sm:flex-row flex-col flex gap-4">
        ...
        <aside className="sm:w-3/12 py-4 rounded-md bg-gray-700 sm:h-screen flex sm:flex-col justify-around sm:justify-normal sm:items-center">
          <div className=''>
            {user && user.image ? (
              <img className="sm:h-40 sm:w-40 h-16 w-16 rounded-full bg-white" src={user.image} alt="profile" />
            ) : (
              <img className="sm:h-40 sm:w-40 h-16 w-16 rounded-full bg-white" src="/profileChar.jpg" alt="profile" />
            )}
        ...
        <div className={`sm:w-6/12 bg-gray-700 h-screen relative rounded-md ${activeSection === 'about' ? 'overflow-y-auto scrollbar-thin-transparent' : ''}`}>
          <div className="flex justify-between">
            <nav className="flex gap-12 bg-gray-600 py-4 rounded-md w-full justify-center z-50">
              <button className={`${activeSection === 'about' ? 'underline underline-offset-8 decoration-2 decoration-white' : ' transition-all duration-1000 ease-out'}`} onClick={() => setActiveSection('about')}>About</button>
              <button className={`${activeSection === 'chatwindow' ? 'underline underline-offset-8 decoration-2 decoration-white' : ' transition-all duration-1000 ease-out'}`} onClick={() => setActiveSection('chatwindow')}>Conversation</button>
            </nav>
          </div>
          <div className="p-8">
            <TransitionGroup>
              <CSSTransition
                key={activeSection}
                nodeRef={activeSection === 'about' ? aboutRef : chatwindowRef}
                timeout={200}
                classNames="fade"
              >
                <div ref={activeSection === 'about' ? aboutRef : chatwindowRef}>
                  {renderSection()}
                </div>
              </CSSTransition>
            </TransitionGroup>
